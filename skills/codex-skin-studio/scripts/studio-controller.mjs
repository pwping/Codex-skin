...
// Windows: WebSocket compatibility layer (native in Node 22+, ws package fallback)
import { createRequire } from 'node:module';
const _nodeRequire = createRequire(import.meta.url);
let _WebSocket;
try { _WebSocket = globalThis.WebSocket; if (!_WebSocket) throw new Error('no native'); } catch { try { _WebSocket = _nodeRequire('ws'); } catch { throw new Error('WebSocket unavailable. Install Node.js 22+ or run: npm install ws'); } }
...
function parseArgs(argv) {
  ...
  for (let index = 1; index < argv.length; index += 1) {
    ...
    else if (arg === "--codex-bin") options.codexBin = path.resolve(argv[++index]);
...
class CdpSession {
  constructor(target, port, transportTarget = target, attachTargetId = null, commandTimeoutMs = 10000) {
    ...
    this.socket = new _WebSocket(validatedWebSocketUrl(transportTarget, port));
    this.nextId = 1;
...
async function resolveCodexBin(options) {
  if (options.codexBin) {
    try { await fs.access(options.codexBin); return options.codexBin; }
    catch { throw new Error(`Codex CLI not found at: ${options.codexBin}`); }
  }
  
  // Try common locations on Windows
  const candidates = [];
  
  // Check WindowsApps for Codex
  const { execSync } = await import("node:child_process");
  try {
    const whereResult = execSync("where codex 2>nul", { shell: "cmd.exe", timeout: 5000, encoding: "utf8" });
  ...
  // Try copying from known WindowsApps location
  const knownPaths = [];
  try {
    const appsDir = "C:\\Program Files\\WindowsApps";
    const dirs = await fs.readdir(appsDir);
    const codexDirs = dirs.filter(d => d.startsWith("OpenAI.Codex_")).sort().reverse();
    if (codexDirs.length > 0) {
      knownPaths.push(path.join(appsDir, codexDirs[0], "app", "resources", "codex.exe"));
    }
  } catch {}
  
  for (const src of knownPaths) {
    try {
      await fs.access(src);
      // Copy to temp location to bypass AppLocker
      const tmpDir = path.join(process.env.TEMP || os.tmpdir(), "CodexSkinStudio");
      await fs.mkdir(tmpDir, { recursive: true });
      const dst = path.join(tmpDir, "codex-cli.exe");
      await fs.copyFile(src, dst);
      return dst;
    } catch {}
  }
  
  throw new Error("Codex CLI not found. Provide --codex-bin path or install Codex.");
}
function buildTextOnlyPrompt(exported, boldness) {
  const analysis = exported.analysis || {};
  const swatches = (analysis.swatches || []).join(", ");
  const boldnessNames = { subtle: "subtle-restrained", wild: "wild-expressive", crazy: "crazy-maximal" };
  
  return `You are a Codex Open Design specialist. Create a structured theme design from the following image color analysis. The output must be valid JSON matching the provided schema exactly.

Color Analysis:
- Dominant Hue: ${analysis.hue || "unknown"} degrees
- Chroma (saturation): ${analysis.chroma || "unknown"}
- Accent Hue: ${analysis.accentHue || "unknown"} degrees  
- Average Lightness: ${analysis.averageLightness || "unknown"}
- Contrast Score: ${analysis.contrastScore || "unknown"}
- Colorfulness: ${analysis.colorfulness || "unknown"}
- Focal Point: ${analysis.focalX || 50}% x, ${analysis.focalY || 50}% y
- Color Swatches: ${swatches}

Image dimensions: ${exported.theme?.imageWidth || "?"}x${exported.theme?.imageHeight || "?"}

Boldness level: ${boldness} (${boldnessNames[boldness] || boldness}). Design a complete interface theme using these colors.

CRITICAL: Output ONLY valid JSON matching the provided schema. Do not include markdown code blocks or extra text. The output must be parseable as JSON directly.`;
  ...
  // Windows: use --codex-bin or auto-detect
  const codexBin = await resolveCodexBin(options);
  // codexBin resolved via resolveCodexBin()
  ...
  try {
    ...
    // Use resolved codexBin path
    // Try image-based generation first, fall back to text-only for non-vision models
    let generation;
    try {
      generation = execFileAsync(codexBin, [
        "exec",
        "--ephemeral",
        "--skip-git-repo-check",
        "--ignore-rules",
      ...
      const errMsg = String(firstError.message || firstError.stderr || "");
      if (errMsg.includes("image_url") || errMsg.includes("vision") || errMsg.includes("invalid_request_error")) {
        await notifyGeneration(session, {
          nonce: command.nonce,
          state: "running",
          message: "模型不支持图片输入，正在用色彩数据生成设计方案...",
        });
        const textPrompt = buildTextOnlyPrompt(exported, boldness);
        generation = execFileAsync(codexBin, [
          "exec",
          "--ephemeral",
          "--skip-git-repo-check",
